		 ...
		 //C0672
		  ...
		  if (c.Status != 'Closed')
		  {
			...
			if((c.CreatedDate != null && c.CreatedDate > thresholdDate) && (Datetime.now() > thresholdDate))
			{
						 ...
						 (beforeValue == null || (beforeValue != null  && !beforeValue.equalsIgnoreCase(afterValue))))
						 {
							//C0672
								...
								if (afterValue.startsWith('00G'))
								{
								 ...
								 c.Internal_SLA__c = CaseTriggerMethods.CalculateInternalSla(bh);
				...
				Internal_Case_Audit__c breachedAuditTeam = [select id, Team__c, New_Owner__c, Breached_external_SLA__c from Internal_Case_Audit__c where Case__c =:c.Id and Breached_external_SLA__c =: true order by CreatedDate desc LIMIT 1];
				if (breachedAuditTeam.Breached_external_SLA__c == true)
				{
